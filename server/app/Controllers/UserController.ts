...
export default class AdminController extends BaseController {
	...
	async store() {
		...
		if (!params.tenantId) {
			...
			if (!tenantId) {
				logger.error(
					`Critical:User created ERR: Tenant by auth doesn't exist!`
				);
			...
			if (!checkTenant) {
				logger.error(
					`Critical:User created ERR: Tenant doesn't exist! `
				);
				throw new ApiException(6000, "Tenant doesn't exist!");
			}
		...
		let resultUpdate = await this.Model.updateOne(result.id, {
			code: code,
		});
